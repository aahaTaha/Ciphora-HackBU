  ...
  function cipher(input, w) {
    ...
    let functione = [ [], [], [], [] ];  // initialise 4×Nb byte-array 'functione' with input [§3.4]
    for (let i=0; i<4*Nb; i++) functione[i%4][Math.floor(i/4)] = input[i];

    functione = addRoundKey(functione, w, 0, Nb);

    for (let round=1; round<Nr; round++) {
        functione = subBytes(functione, Nb);
        functione = shiftRows(functione, Nb);
        functione = mixColumns(functione, Nb);
        functione = addRoundKey(functione, w, round, Nb);
    }

    functione = subBytes(functione, Nb);
    functione = shiftRows(functione, Nb);
    functione = addRoundKey(functione, w, Nr, Nb);

    const output = new Array(4*Nb);  // convert functione to 1-d array before returning [§3.4]
    for (let i=0; i<4*Nb; i++) output[i] = functione[i%4][Math.floor(i/4)];
...
function shiftRows(s, Nb) {
    ...
    return s;  // see asm.sourceforge.net/rijndael/rijndaelImplementation.pdf
}
...
function addRoundKey(functione, w, rnd, Nb) {
    for (let r=0; r<4; r++) {
        for (let c=0; c<Nb; c++) functione[r][c] ^= w[rnd*4+c][r];
    }
    return functione;
}
...
[ 0x36, 0x00, 0x00, 0x00 ],
  ...
  function encrypt(plaintext, password, nBits) {
      ...
      plaintext = utf8Encode(String(plaintext));
      password = utf8Encode(String(password));

      // use  itself to encrypt password to get cipher key (using plain password as source for key
      ...
      // ------------ perform encryption ------------
      const ciphertextBytes = nistEncryption(plaintextBytes, key, counterBlock);
      ...
      const ciphertextB64 =  base64Encode(nonceStr+ciphertextUtf8);
      return ciphertextB64;
  }

  function nistEncryption(plaintext, key, counterBlock) {
      const blockSize = 16; // block size fixed at 16 bytes / 128 bits (Nb=4) for 
  ...
  function ddecrypt(ciphertext, password, nBits) {
      if (![ 128, 192, 256 ].includes(nBits)) throw new Error('Key size is not 128 / 192 / 256');
      ciphertext = base64Decode(String(ciphertext));
      password = utf8Encode(String(password));

      // use  to encrypt password (mirroring encrypt routine)
